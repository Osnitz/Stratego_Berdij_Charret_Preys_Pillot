...
BOOST_AUTO_TEST_SUITE(PlayerControllerTestSuite)
...
BOOST_AUTO_TEST_CASE(TestGetScenarioChoice)
{
    ...
    // Case 3: User chooses AIvsAI (3) with AI difficulties 1 and 2
    {
    ...
    // Case 4: Invalid initial input
    {
        std::istringstream input("0\n4\n1\n");
        std::cin.rdbuf(input.rdbuf()); // Redirect std::cin to use input stream
        GameMode mode = scenarioManager->getScenarioChoice();
        BOOST_CHECK(mode == GameMode::PVP); // Default to PvP after valid input
    }
